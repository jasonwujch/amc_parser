#!/usr/bin/env python3
"""
AMC12 Solutions Parser
- Parses solutions for AMC 12A and AMC 12B (2002-2025)
- Smart resume: checks existing files and skips them
- Progress tracking
- 25 problems per contest (multiple choice A-E)
"""
...
def parse_solution_page(html_content, problem_num, year, variant, season=None):
    ...
    for element in content_div.find_all(['p', 'h2', 'h3', 'h4', 'div', 'ul', 'ol']):
        if element.name in ['h2', 'h3', 'h4']:
            ...
            # Handle lists (common in solutions)
            list_items = []
...
def check_existing_files(base_path):
    ...
    existing_files = {}
    amc12_dir = Path(base_path) / "AMC12"

    if not amc12_dir.exists():
...
def main():
    ...
    if not incomplete_contests and not missing_contests:
        print("\nAll AMC12 solutions are already complete!")
        return
