#!/usr/bin/env python3
"""
AMC 8 Solutions Parser - Smart Resume Edition
- Checks existing files and skips them
...
def check_existing_files(base_path):
    ...
    existing_files = {}
    amc8_dir = Path(base_path) / "AMC8"
    
    if not amc8_dir.exists():
        return existing_files
    
    for year_dir in amc8_dir.iterdir():
        if year_dir.is_dir() and "AMC 8" in year_dir.name:
            # Extract year from directory name like "2007 AMC 8"
            year_match = re.match(r'(\d{4}) AMC 8', year_dir.name)
            if year_match:
                ...
                if solutions_dir.exists():
                    ...
                    for solution_file in solutions_dir.glob(f"{year}_amc_8_solution_p*.md"):
                        ...
                        if problem_match:
                            ...
                            existing_files[year].add(problem_num)
...
def analyze_progress(existing_files):
    ...
    for year in range(1999, 2026):  # 1999-2025
        if year in existing_files:
            existing_count = len(existing_files[year])
            missing_count = 25 - existing_count
            total_existing += existing_count
            total_missing += missing_count
            
            if existing_count == 25:
                print(f"  {year}: ✅ Complete (25/25)")
            else:
                print(f"  {year}: ⚠️  Incomplete ({existing_count}/25) - missing {missing_count} problems")
                missing_problems = set(range(1, 26)) - existing_files[year]
                print(f"    Missing problems: {sorted(missing_problems)}")
                incomplete_years.append((year, missing_problems))
        else:
            print(f"  {year}: ❌ Not started (0/25)")
            total_missing += 25
    ...
    return incomplete_years, missing_years

def parse_missing_problems(year, missing_problems, base_path):
    """Parse only the missing problems for a given year."""
    base_url = f"https://artofproblemsolving.com/wiki/index.php/{year}_AMC_8_Problems/Problem_"
    solutions_dir = create_directory_structure(base_path, year)
    
    print(f"\n{'='*50}")
    print(f"Parsing missing problems for {year} AMC 8")
    print(f"Missing problems: {sorted(missing_problems)}")
    print(f"{'='*50}")
    ...
    for problem_num in sorted(missing_problems):
        ...
        # Double check if file exists (might have been created by another process)
        if solution_file.exists():
            print(f"⏭️  Skipping Problem {problem_num} (file already exists)")
            continue
    ...
    print(f"\n=== {year} Summary ===")
    print(f"Successfully parsed: {success_count}/{len(missing_problems)} problems")
    if failed_problems:
        print(f"Failed problems: {failed_problems}")
    else:
        print("All missing problems parsed successfully!")
    
    return success_count, failed_problems

def parse_complete_year(year, base_path):
    """Parse all 25 problems for a year that hasn't been started."""
    return parse_missing_problems(year, set(range(1, 26)), base_path)

def main():
    """Main function with smart resume capability."""
    print("AMC 8 Solution Parser - Smart Resume Edition")
    print("Analyzing existing files and determining what needs to be done...\n")
    
    base_path = "data/raw"
    existing_files = check_existing_files(base_path)
    incomplete_years, missing_years = analyze_progress(existing_files)
    
    if not incomplete_years and not missing_years:
        print("\n🎉 All AMC 8 solutions (1999-2025) are already complete!")
        return
    
    print(f"\n📋 WORK TO BE DONE:")
    if incomplete_years:
        print(f"  Incomplete years to finish: {len(incomplete_years)}")
    if missing_years:
        print(f"  Missing years to start: {len(missing_years)}")
    ...
    # First, complete incomplete years
    for year, missing_problems in incomplete_years:
        success_count, failed_problems = parse_missing_problems(year, missing_problems, base_path)
        total_success += success_count
        total_attempted += len(missing_problems)
        
        if failed_problems:
            print(f"⚠️  {year} completed with {len(failed_problems)} failures")
        else:
            print(f"✅ {year} completed successfully!")
        
        time.sleep(2)  # Brief pause between years
    
    # Then, start missing years
    for year in missing_years:
        print(f"\n{'='*50}")
        print(f"Starting {year} AMC 8 (complete year)")
        print(f"{'='*50}")
        
        success_count, failed_problems = parse_complete_year(year, base_path)
        total_success += success_count
        total_attempted += 25
    ...
    if total_success == total_attempted:
        print("\n🎉 All AMC 8 solutions (1999-2025) are now complete!")
    else:
        print(f"\n⚠️  Completed with {total_attempted - total_success} failures")
