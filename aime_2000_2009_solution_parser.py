...
- Parses solutions for AIME I and II (2000-2009)
...
def clean_solution_content(content):
    ...
    for line in lines:
        ...
        # Skip duplicate copyright notices (keep only the first one)
        if 'copyrighted' in line.lower() or 'copyright' in line.lower():
            if seen_copyright:
                continue
            seen_copyright = True
...
def check_existing_files(base_path):
    ...
    for year_dir in aime_dir.iterdir():
        if year_dir.is_dir() and "AIME" in year_dir.name:
            # Extract year and variant from directory name like "2000 AIME I"
