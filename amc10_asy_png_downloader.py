#!/usr/bin/env python3
"""
AMC10 ASY PNG Downloader - Download PNG images for Asymptote diagrams
Downloads rendered PNG images for all [asy]...[/asy] blocks from AMC10 problems.
Covers AMC 10A and 10B from 2002-2025, including 2021 Fall variants.
"""
...
def get_contest_key(year, variant):
    """Get a filesystem-safe key for the contest."""
    if variant.startswith('Fall_'):
        letter = variant.replace('Fall_', '')
        return f"{year}_Fall_AMC_10{letter}"
    else:
...
def download_asy_images_for_contest(year, variant, output_base_dir, existing_manifest):
    """Download ASY PNG images for a specific AMC10 contest."""
    url = get_url_for_variant(year, variant)
    contest_name = get_display_name(year, variant)
    contest_key = get_contest_key(year, variant)
...
def main():
    """Download ASY PNG images for all AMC10 contests 2002-2025."""
    print("=" * 60)
    print("AMC10 ASY PNG Downloader")
    print(f"Downloading PNG images for ASY blocks from {START_YEAR}-{END_YEAR}")
    print("=" * 60)

    output_base_dir = Path("data/raw/AMC10/asy_images")
    ...
    for year in range(END_YEAR, START_YEAR - 1, -1):
        ...
        for variant in variants:
            ...
            try:
                ...
                print(f"  Error processing {get_display_name(year, variant)}: {e}")
                results[contest_key] = {
                    'images': [],
                    'count': 0,
                    'complete': False,
