#!/usr/bin/env python3
"""
AIME ASY PNG Downloader - Download PNG images for Asymptote diagrams
Downloads rendered PNG images for all [asy]...[/asy] blocks from AIME problems.
Covers AIME I and AIME II from 2000-2009.
"""
...
def extract_asy_images(html_content, year, variant):
    ...
    for img in all_imgs:
        ...
        if alt_text and alt_text.startswith('[asy]') and alt_text.endswith('[/asy]'):
            ...
            problem_num = None
            img_position = str(img.sourceline) if hasattr(img, 'sourceline') else '0'
...
def download_asy_images_for_contest(year, variant, output_base_dir, existing_manifest):
    """Download ASY PNG images for a specific AIME contest."""
    ...
    contest_key = f"{year}_AIME_{variant}"
    ...
    asy_images = extract_asy_images(html_content, year, variant)
    ...
    output_dir = output_base_dir / f"{year}_AIME_{variant}"
...
def main():
    """Download ASY PNG images for all AIME contests 2000-2009."""
    print("=" * 60)
    print("AIME ASY PNG Downloader")
    print("Downloading PNG images for ASY blocks from 2000-2009")
    print("=" * 60)

    output_base_dir = Path("data/raw/AIME/asy_images")
    ...
    total_images = 0
    results = dict(existing_manifest)  # Preserve existing entries
    ...
    for year in range(end_year, start_year - 1, -1):
        for variant in ['I', 'II']:
            ...
            try:
                ...
                print(f"  Error processing {year} AIME {variant}: {e}")
                results[contest_key] = {
                    'images': [],
                    'count': 0,
                    'complete': False,
    ...
    print(f"Total ASY images downloaded for 2000-2009: {total_images}")
    print(f"Manifest saved to: {manifest_path}")
    print("=" * 60)

    # Print summary for 2000-2009 only
    print("\nSummary by contest (2000-2009):")
    for year in range(end_year, start_year - 1, -1):
        for variant in ['I', 'II']:
            contest_key = f"{year}_AIME_{variant}"
            if contest_key in results:
